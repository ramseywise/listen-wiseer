...
def _make_conn() -> duckdb.DuckDBPyConnection:
    ...
    init_schema(conn)
    # Populate genre_xy with a small set
    conn.execute("""
...
class TestSyncLastfmGenres:
    def test_skips_when_no_api_key(self):
        ...
        with patch("etl.sync.settings") as mock_settings:
            ...
            result = sync_lastfm_genres(conn)
        assert result == 0

    def test_skips_when_genre_xy_empty(self):
        conn = duckdb.connect(":memory:")
        init_schema(conn)
        # genre_xy is empty (no bootstrap)
        with patch("etl.sync.settings") as mock_settings:
            mock_settings.lastfm_api_key = "somekey"
            result = sync_lastfm_genres(conn)
        assert result == 0

    def test_writes_first_genre_and_stub_audio_features(self):
        conn = _make_conn()
        conn.execute("""
        ...
        assert result == 1
        genre = conn.execute("SELECT first_genre FROM tracks WHERE track_id = 't1'").fetchone()[0]
        assert genre == "jazz"
        src = conn.execute(
    ...
    def test_no_match_leaves_first_genre_null(self):
        conn = _make_conn()
        conn.execute("""
            INSERT INTO tracks (track_id, track_name, release_date)
        ...
        assert result == 0
        genre = conn.execute("SELECT first_genre FROM tracks WHERE track_id = 't2'").fetchone()[0]
        assert genre is None
        af = conn.execute("SELECT track_id FROM audio_features WHERE track_id = 't2'").fetchone()
        assert af is None

    def test_limit_caps_tracks_processed(self):
        conn = _make_conn()
        for i in range(5):
            conn.execute(
        ...
        with (
            ...
            sync_lastfm_genres(conn, limit=2)

        # Only 2 tracks passed to fetch
        call_args = mock_fetch.call_args[0][0]
        assert len(call_args) == 2

    def test_does_not_overwrite_existing_audio_features(self):
